...
func (d *Driver) StreamEvents(ctx context.Context, sink chan<- dahua.Event) error {
	...
	for {
		...
		resp, err := d.cgi.OpenStream(ctx, "/cgi-bin/eventManager.cgi", query)
		if err != nil {
			return err
		}
		...
		d.logger.Warn().Err(parseErr).Msg("ipc event stream disconnected, retrying")
		select {
		case <-ctx.Done():
			return ctx.Err()
...
func normalizeEvent(rootID string, values map[string]string) (dahua.Event, bool) {
	...
	index := parseIndex(firstNonEmpty(values["index"], values["Index"]), 0)
	channel := index + 1
	if channel <= 0 {
		channel = 1
	}
