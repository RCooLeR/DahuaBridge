...
func (c *rpcClient) Call(ctx context.Context, method string, params any, target any) error {
	...
	if err := c.callOnce(ctx, vtoRPCPath, c.buildRequest(method, params), target); err != nil {
		if rpcErr, ok := err.(*rpcError); ok && isAuthRPCError(rpcErr.Code) {
			c.resetSession()
			if loginErr := c.ensureLogin(ctx); loginErr != nil {
				return loginErr
			}
			return c.callOnce(ctx, vtoRPCPath, c.buildRequest(method, params), target)
		}
		return err
	}
...
func (c *rpcClient) post(ctx context.Context, path string, req rpcRequest, target *rpcResponse) error {
...
func (c *rpcClient) buildRequest(method string, params any) rpcRequest {
	return rpcRequest{
		Method:  method,
		Params:  params,
		ID:      c.nextRequestID(),
