...
func TestBuildCatalogForVTOIncludesLockCount(t *testing.T) {
	...
	if catalog[0].Intercom.SupportsVTOCallAnswer || catalog[0].Intercom.SupportsVTOTalkback || catalog[0].Intercom.SupportsFullCallAcceptance {
		t.Fatalf("expected unsupported end-to-end intercom features, got %+v", catalog[0].Intercom)
	}
}
